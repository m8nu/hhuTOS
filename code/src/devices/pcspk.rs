...
pub fn play(f: f32, d: u32) {
    ...
    outb(PORT_CTRL, 0xb6); 
    let temp = (1193182.0/f) as u16;
    outb(PORT_DATA2,  (temp & 0xFF) as u8);
    outb(PORT_DATA2, ((temp >> 8) & 0xFF) as u8);
...
fn read_counter() -> u32 {
   ...
   /* Hier muss Code eingefuegt werden */
    outb(PORT_CTRL, 0x00);
    let low = inb(PORT_DATA0) as u32;
    let high = inb(PORT_DATA0) as u32;
    return (high << 8) | low;
}
...
fn delay(d: u32) {
    ...
    outb(PORT_DATA0, (freq & 0xFF) as u8);
    outb(PORT_DATA0, ((freq >> 8) & 0xFF) as u8);

    let mut c = d + 1;
    let mut c_temp = d as i64;

    while  c_temp > 0 {
       let new = read_counter();
       if new > c {
            c_temp -= 10;
       }
       c = new;
